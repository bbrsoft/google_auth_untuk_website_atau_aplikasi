...
try {
    ...
    if (!empty($emails)) {
        $firstMessageId = $emails[0]['id']; // Ambil email pertama
    
        try {
            // **Pindahkan dari Spam ke Inbox**
            $service->users_messages->modify("me", $firstMessageId, new Google\Service\Gmail\ModifyMessageRequest([
                "removeLabelIds" => ["SPAM"],
                "addLabelIds" => ["INBOX"]
            ]));
            ...
            echo json_encode([
                "status" => "success",
                "message" => "Email pertama berhasil dipindahkan dari Spam ke Inbox.",
                "email_moved" => $emails[0]
            ]);
        } catch (Exception $e) {
            echo json_encode(["error" => "Gagal memindahkan email: " . $e->getMessage()]);
        }
    } else {
        echo json_encode(["message" => "Tidak ada email spam untuk dipindahkan."]);
    }
