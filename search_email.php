...
try {
    $query = [];
    if (isset($_GET['from'])) {
        $query['q'] = 'from:' . $_GET['from'];
    }

    $messages = $service->users_messages->listUsersMessages("me", array_merge(['maxResults' => 10], $query));
    ...
    if (!empty($emails)) {
        if (isset($_GET['from'])) {
            echo json_encode(["emails" => $emails]); // Jika cari berdasarkan pengirim
        } else {
            $randomEmail = $emails[array_rand($emails)]; // Pilih satu email secara random
            echo json_encode(["random_email" => $randomEmail]);
        }
    } else {
        echo json_encode(["error" => "Tidak ada email ditemukan sesuai filter."]);
    }
} catch (Exception $e) {
